...
#
# Usage (environment):
#   export VPS_USER=you VPS_HOST=203.0.113.50 REMOTE_PORT=8892 LOCAL_PORT=8040 SSH_PORT=22
#   bash scripts/macmini-reverse-tunnel.sh
#
# If GoDaddy uses a non-default SSH port, set SSH_PORT (e.g. 2222) or pass as 5th argument.
#
...
if [[ -z "${VPS_USER}" || -z "${VPS_HOST}" ]]; then
  ...
  echo ""
  echo "Usage:"
  echo "  $0 <vps_ssh_user> <vps_ip_or_hostname> [remote_port_on_vps] [local_api_port_on_mac] [ssh_port]"
  echo ""
  echo "Example:"
  echo "  $0 deployuser 203.0.113.50 8892 8040 22"
  echo ""
  echo "From your git clone (script is inside the repo, not ~/scripts by default):"
  echo "  bash ~/multi-model-ml-platform/scripts/macmini-reverse-tunnel.sh USER HOST 8892 8040"
  echo ""
  echo "Or set env vars then run with no args:"
  echo "  export VPS_USER=deployuser VPS_HOST=203.0.113.50 REMOTE_PORT=8892 LOCAL_PORT=8040 SSH_PORT=22"
  echo "  $0"
  echo ""
...
if command -v autossh >/dev/null 2>&1; then
  echo "[$(date '+%Y-%m-%d %H:%M:%S')] Using autossh → ${VPS_USER}@${VPS_HOST}:${SSH_PORT}  -R ${R_SPEC}"
...
echo "Running: ssh -p ${SSH_PORT} ... -R ${R_SPEC} ${VPS_USER}@${VPS_HOST}"
